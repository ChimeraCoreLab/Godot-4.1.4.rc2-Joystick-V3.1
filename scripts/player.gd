...
@onready var footstep_audio : AudioStreamPlayer3D = get_node(NodePath("Node3D/footstep2"))
...
func _play_footstep_audio():
	footstep_audio.pitch_scale = randf_range(.1, 1)
	footstep_audio.play()
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch") or $RayCast.is_colliding() or $"../UI/crouch".is_pressed():
		if is_on_floor():
			current_speed = lerp(current_speed, CROUCHING_SPEED, delta * LERP_SPEED)
		$StandingCollisionShape/MeshInstance3D.hide()
		$CrouchingCollisionShape/MeshInstance3D.show()
		...
		$Neck/Head.position.y = lerp($Neck/Head.position.y, 0.0, delta * LERP_SPEED)
		$StandingCollisionShape/MeshInstance3D.show()
		$CrouchingCollisionShape/MeshInstance3D.hide()
		$StandingCollisionShape.disabled = false
		$CrouchingCollisionShape.disabled = true
		$SlidingTimer.stop()
		if Input.is_action_pressed("sprint") or $"../UI/RunBtn".is_pressed():
	...
	if Input.is_action_pressed("free_look") or !$SlidingTimer.is_stopped() or $"../UI/freelook".is_pressed():
	...
	if Input.is_action_pressed("jump") or $"../UI/JumpBtn".is_pressed() and is_on_floor():
